...
func (p *pgTester) setup() error {
	...
	viper.SetDefault("pgx.schema", "public")
	viper.SetDefault("pgx.port", 5432)
	viper.SetDefault("pgx.sslmode", "require")

	p.dbName = viper.GetString("pgx.dbname")
	p.host = viper.GetString("pgx.host")
	p.user = viper.GetString("pgx.user")
	p.pass = viper.GetString("pgx.pass")
	p.port = viper.GetInt("pgx.port")
	p.sslmode = viper.GetString("pgx.sslmode")
	p.testDBName = viper.GetString("pgx.testdbname")
	p.skipSQLCmd = viper.GetBool("pgx.skipsqlcmd")

	err = vala.BeginValidation().Validate(
		vala.StringNotEmpty(p.user, "pgx.user"),
		vala.StringNotEmpty(p.host, "pgx.host"),
		vala.Not(vala.Equals(p.port, 0, "pgx.port")),
		vala.StringNotEmpty(p.dbName, "pgx.dbname"),
		vala.StringNotEmpty(p.sslmode, "pgx.sslmode"),
	).Check()
	...
	if !p.skipSQLCmd {
		...
		if err = createCmd.Start(); err != nil {
			return errors.Wrap(err, "failed to start pgx command")
		}
		...
		if err = createCmd.Wait(); err != nil {
			...
			return errors.Wrap(err, "failed to wait for pgx command")
		}
	}
...
func (p *pgTester) conn() (*sql.DB, error) {
	...
	var err error
	p.dbConn, err = sql.Open("pgx", driver.PSQLBuildQueryString(p.user, p.pass, p.testDBName, "", p.host, p.port, p.sslmode))
	if err != nil {
		return nil, err
	}
