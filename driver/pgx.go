...
func init() {
	drivers.RegisterFromInit("pgx", &PostgresDriver{})
}
...
func (p *PostgresDriver) Assemble(config drivers.Config) (dbinfo *drivers.DBInfo, err error) {
	...
	p.connStr = PSQLBuildQueryString(user, pass, dbname, schema, host, port, sslmode)
	p.conn, err = sql.Open("pgx", p.connStr)
	if err != nil {
		return nil, errors.Wrap(err, "sqlboiler-pgx failed to connect to database")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "sqlboiler-pgx failed to get database version")
	}

	if err = p.loadUniqueColumns(); err != nil {
		return nil, errors.Wrap(err, "sqlboiler-pgx failed to load unique columns")
	}
...
func PSQLBuildQueryString(user, pass, dbname, schema, host string, port int, sslmode string) string {
	connectionUrl := url.URL{}
	connectionUrl.Scheme = "postgres"
	connectionUrl.User = url.UserPassword(user, pass)
	connectionUrl.Host = fmt.Sprintf("%s:%d", host, port)
